...
require(['jquery', 'ymt', 'placeholder', 'tracker', 'entrust', 'mar', 'cookie'], function ($, Y){
    ...
    //公共广告位
    (function(){
        if(!$('body').data('isblack')){
            var mir = $('.content');
            if(mir.length > 0){
                $.ajax({
                    type: "POST",
                    url: "http://www.ymt.com/ajax/getcommonrightdata",
                    dataType: "jsonp",
                    jsonp:"callback",
                    success:function(msg){
                        var html = '<li style="position: relative;margin-top: 10px;">';
                        if(typeof msg.right_reporter_apply_60.img_url != 'undefined'){
                            html += '<a class="reporter_pic" target="_blank" href="'+msg.right_reporter_apply_60.jump_url+'"><img src="'+msg.right_reporter_apply_60.img_url+'"/></a>';
                        }
                        if(msg.status != "undefined" && msg.status == 2){
                            if(typeof msg.right_reporter_apply_250.img_url != 'undefined'){
                                html += '<div id="propagates_pic" style="position: absolute;left: -260px;z-index: 1;bottom:0px;">' +
                                    '<a href="'+msg.right_reporter_apply_250.jump_url+'" target="_blank">' +
                                    '<img src="'+msg.right_reporter_apply_250.img_url+'"/>' +
                                    '</a>' +
                                    '<a id="closereporter" href="###"></a>' +
                                    '</div>';
                            }
                            html += '</li>';
                            $('.fc_2').append(html);
                ...
                $('#reporter_small_div').css('left', (mir.offset().left + mir.width() + 10) + 'px');
                var first = true,
                    $reporterDiv = $('#reporter_small_div'),
                    mirtop = $(".content").offset().top;
                $(window).scroll( function() {
                    if( $(window).scrollTop() > mirtop) {
                        if(first){
                            $reporterDiv.show();
                            first = false;
                        }
                        $reporterDiv.css('left', (mir.offset().left + mir.width() + 10) + 'px');
                        $reporterDiv.css({position:"fixed",bottom:"10px",top:"none"});
                    }else {
                        first = true;
                        $reporterDiv.hide();
                        $reporterDiv.css("position","absolute");
                        $reporterDiv.attr("style",{"bottom":"none"});
                        $reporterDiv.css('left', (mir.offset().left + mir.width() + 10) + 'px');
                    }
                });

                $(".fc_2").on("click","#closereporter",function(){
                    $("#propagates_pic").hide();
                    $.ajax({
                        type: "POST",
                        url: "http://www.ymt.com/ajax/setadvertcookie",
                        dataType: "jsonp",
                        jsonp:"callback",
                        success:function(msg){}
                    });
