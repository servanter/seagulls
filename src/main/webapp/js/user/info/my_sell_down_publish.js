$(function() {
	...
	$('#a-on').click(function() {
		...
		$.getJSON(BaseUtils.proPath + 'user/sell/on/?detail_ids=' + ids, function(data) {
			if (data.code != 10000) {
...
function reload() {
	...
	$.getJSON(BaseUtils.proPath + 'user/sell/ajaxMyDown/?page=1', function(data) {
		if (data.code != 10000) {
			Alert.info(data.message);
		} else {
			$('#wrapper ul').empty();
...
function nextPage() {
	...
	$.getJSON(BaseUtils.proPath + 'user/sell/ajaxMyDown/?page=' + curPage, function(data) {
		if (data.code != 10000) {
			Alert.info(data.message);
		} else {
			curPage = data.result.nextPage;
